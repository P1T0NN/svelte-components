...
**Which factory?** Paginated *list* for `ConvexDataTable` / infinite scroll →
`fetchOptimized`. Dropdown/autocomplete *suggestions* → `createSearchQuery`
...
At most **one** mode may resolve per request. All of them can be *defined* on one
...
Runs on the *already-paginated page* (≤ `numItems` rows), so it adds at most O(perPage)
...
- `minQueryLength` (default 2) — short/empty queries return an empty page before
  touching the index *or* the rate-limit bucket;
